...
- 2026-09-23. Added `memStrain` (membrane only, default 0): a membrane bond whose pinned corners the solver leaves further apart than a threshold lets go. Reason: the user's picture of bonded polygons as flush, one shape with a line between; the pins are constraints solved in a fixed number of passes, and in a shape the blocks cannot fit (a ring with more or fewer wedges than its bend closes) they settle on an even compromise with gaping corners, which more passes do not remove. With the threshold at 0.25 a side, only rings that fit last; an overgrown ring sheds its excess and recloses (`experiments/RESULTS.md`, section 16). Not applied to replicator bonds: bonds breaking from mechanics would change a core commitment (section 2), and that is the user's call.
