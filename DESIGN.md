...
- **Blocks are conserved.** No block is ever created or destroyed during a run (the user, 2026-09-25: blocks appearing out of nowhere would make replication part of the pre-programmed instead of emergent). Every copy is built from monomers that were already there, and no rule copies, duplicates or inserts anything: a duplication or an insertion can only come out of the existing physics (a copy bridging two templates, broken pieces rejoined by ligation).
...
- 2026-09-24. Chirality (`chiral`, `pMisDock`, `pMixLink`, `pRacem`), droplets (`nG`, `gStick`, `gRange`, `gStickS`, `gStickF`) and random chemistry (`src/rchem.js`), from the user's question which block types reality's chemistry and microbiology suggest, and whether blocks with random sides and random switching rules would give chaos or complexity. Chirality makes hand a fixed per-block property like type (a mirror letter is written lowercase); the only change of hand is racemization of a free monomer (`pRacem`), a spontaneous state flip, not a block turning into another type. Droplets add an attraction between neighbours (no bonds), the first force in the engine that is not a contact, a pin or a shape force. Random chemistry keeps the physics and swaps the rule table for a random one of the same locality (a side's colour from its block's type, state and side; switches triggered by a bonded side's colour), so the hand-made table is one point of the space being searched. Evidence in `experiments/RESULTS.md`, sections 30 to 32.
- 2026-09-24. Locality made the fundamental commitment (the user: it is why the project is interesting; section 2). Two consequences the same day. First, relayed signals (`relay`, `cutRelay`, `tether`) are read from the previous derive pass (`ss0`), so a signal moves one block per pass: before, one pass could carry it along a whole strand when the blocks happened to be visited in order, which is a strand-level effect. Default trajectory unchanged; relay runs measured before (sections 19, 22, 26) had the faster relay. Second, a first version of the rule below (`whole`: caps sent signals along the strand and a face worked only where both arrived) was replaced before it was measured, because although each step was local, it was written to make whole strands special.
...
- *Chirality* (done, `chiral`): two mirror forms of each letter that do not pair with each other; a population has to break the symmetry.
- *Coacervates* (done, `nG`): liquid droplets that gather strands without a membrane (Oparin).
- *Random chemistries* (done, `src/rchem.js`): the rule table itself drawn at random (Squirm3, Kauffman's autocatalytic sets).
