...
// Usage: node experiments/arc_split.js <N> <memStrain> [pSwap=0] [seeds=20] [memAngle=30]
const { Sim, L, R, T_M } = require('../src/sim.js');
const N = Number(process.argv[2] || 24), strain = Number(process.argv[3] || 0.25), pSwap = Number(process.argv[4] || 0);
...
for (let seed = 1; seed <= seeds; seed++) {
  const s = new Sim({ seed, W: 30, H: 30, nA: 0, nB: 0, nE: 0, nM: N, memAngle: ang, stiffM: 1, pReload: 0, pSwap, memStrain: 0 });
  const r = N / (2 * Math.PI);
  ...
  s._deriveAll(); s._computeOpen(); s.run(300); s.p.memStrain = strain; s.run(20000);
...
console.log(`arc of ${N}, bend ${ang}, memStrain ${strain}, pSwap ${pSwap}, ${seeds} seeds: ${ringsTot} rings, ${(100 * blocksInRings / (N * seeds)).toFixed(0)}% of blocks in rings`);
