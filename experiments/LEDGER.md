...
| `bendA` | 15c (lead), 40f (negative), 49a (works), 49b (lead), 49c (lead), 50a (works), 50b (works), 50c (lead), 51a (negative) |
| `bendB` | 15c (lead), 15d (works), 48 (negative), 49a (works), 49b (lead), 49c (lead), 50a (works), 50b (works), 50c (lead), 51a (negative) |
...
| `pCut` | 26 (negative) |
| `pFray` | 1 (works), 2 (works), 4 (negative), 5b (negative), 12 (works), 13 (works), 13b (works), 33a (partial), 44d (lead), 49c (lead), 50c (lead), 51a (negative), 51b (lead) |
...
| `pUnzip` | 13b (works), 15b (works), 49c (lead), 51b (lead) |
| `radBand` | 33g (lead) |
...
- **Preserve function while recycling incomplete material** (50–51): some curved offspring fully rearm but stall
  among unfinished rows. Turnover permits exact children in two reused seeds while destroying original rows and
  shortening newborns. Test existing end protection on completed and stalled intermediates separately: capping
  a free end could preserve either. Measure lifetimes and copying before a race; keep solver controls and do not
  treat fuel capture, more total births or survival of recycled IDs as sustained inherited function.
