...
console.log('| arm | seed | capped births | mimic share | host occupancy | mimic occupancy | products | same block | mismatches | ready fraction |');
console.log('|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|');
...
for (const arm of [...new Set(rows.map(r=>r.arm))]) {
  ...
  console.log(`${arm}, n=${rs.length}: births ${mean('births').toFixed(1)}, mimic share ${pct(mean('mimicShare'))}, host occupancy ${pct(mean('hostOccupancy'))}, mimic occupancy ${pct(mean('mimicOccupancy'))}, same block ${pct(mean('sameSite'))}`);
}
